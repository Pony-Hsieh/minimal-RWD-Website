<template>
    ...
    <button type="button" class="btn btn-lg btn-dark" @click="ls_Remove_Test">ls Remove Test</button> -->
        ...
        <button type="button" class="btn btn-lg btn-dark"
            @click="multi_AddProductToCart">multi_AddProductToCart</button>
        ...
        methods: {
            ...
            multi_AddProductToCart() {
                ...
                let multiArr = [
                    {
                        // 男士 西裝外套 灰
                        product_id: "-MPseqW11lTu_HkgXL2j",
                        qty: 1,
                    },
                    {
                        // 男士 西裝外套 藍
                        product_id: "-MQgUVc77Cvr4Rxd4gCb",
                        qty: 2,
                    },
                    {
                        // 嘻哈潮流帽
                        product_id: "-MQfy1oYhE8j1LHtdwL1",
                        qty: 30,
                    },
                ];

                localStorage.setItem("userLSCart", JSON.stringify(multiArr));
                let a = localStorage.getItem("userLSCart");
                let b = JSON.parse(a);
                console.log(a);
                console.log(b);
            ...
            getLSCart() {
                ...
                console.log(this.userLSCartArr);
                console.log(this.userLSCartArrNum);
            },


            // 新增 LS 購物車內商品，數量 +1  // 如果 LS 內有相同產品，先刪除，再 +1
            addLSCart(nowProduct) {

                // 將 LS 內的產品 id 取出，一一放入比較用的陣列中
                this.userLSCartArr.forEach((item) => {
                    this.userLSCompareArr.push(item.product_id);
                });

                if (userLSCompareArr.indexOf(nowProduct.product_id) === -1) { // LS內 無 相同產品 // indexOf === -1 代表這個值不存在於陣列中
                    // 直接加入該商品於陣列中，並送往 LS
                    this.userLSCompareArr.push({
                    ...
                    // 再跑一次迴圈，去找相同的產品在哪；加入新的數量於陣列末端；再將原本的資料刪除，並送往 LS
                    this.userLSCartArr.forEach((item, index) => {

                        if (item.product_id === nowProduct.product_id) {
                            this.userLSCartArr.push({
                        ...
                        this.userLSCartArr.splice(index, 1);
                    });
                }
            },
